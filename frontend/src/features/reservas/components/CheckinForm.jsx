...
import { subirFotoCheckin } from "../api/checkinService";
...
const CheckinForm = ({ initialData, onSubmit, isLoading, motivoRechazo }) => {
  ...
  const handleFileUpload = async (e, fieldName) => {
    ...
    try {
      const result = await subirFotoCheckin(file);
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: 800, margin: "0 auto", p: 3, bgcolor: "var(--panel-2)", borderRadius: 4, boxShadow: "var(--shadow-autospot-soft)" }}>
      <Typography variant="h5" sx={{ mb: 4, fontWeight: 700, color: "var(--text)" }}>
        Registro del Estado Inicial (Check-in)
      </Typography>

      {motivoRechazo && (
        <Alert severity="error" sx={{ mb: 4 }}>
          <strong>Check-in Rechazado:</strong> {motivoRechazo}
          <br/>Por favor, corrija la información y vuelva a enviar.
        </Alert>
      )}
